...
export const App = () => {
  ...
  const [contacts, setContacts] = useState(
    () => localStorageCustom.load(LS_KEY) || []
  );

  useEffect(() => localStorageCustom.save(LS_KEY, contacts), [contacts]);

  const onSubmitAddInputValue = (name, number) => {
    const nameId = nanoid();

    setContacts([...contacts, { name, number, id: nameId }]);
  };
  ...
  const onClickDeleteContact = evt => {
    setContacts(() =>
      contacts.filter(el => {
    ...
    <>
      <Wrapper>
        <Wrapper>
          <h1>Phonebook</h1>
          <ContactsForm
            onSubmitAddInputValue={onSubmitAddInputValue}
            contacts={contacts}
          />
        </Wrapper>
        <ContactsWrapper>
          <div>
            <h2>Contacts</h2>
            <FilterNames filter={filter} onChangeName={onInputFind} />
          </div>
          <div>
            <ContactsList
              contacts={contacts}
              filter={filter}
              handleDelete={onClickDeleteContact}
            />
          </div>
        </ContactsWrapper>
      </Wrapper>
